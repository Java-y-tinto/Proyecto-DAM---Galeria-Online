// __tests__/cartResolvers.test.js
import { resolvers } from '../graphql/resolvers/resolvers.js';
import { jest } from '@jest/globals';
import { it, describe, beforeEach, expect } from '@jest/globals';
// 2. Mockear el módulo de odooClientInstance
jest.mock('../instances/odooClientInstance', () => ({
...
// 3. Importar las funciones. TypeScript/IDE necesita ayuda aquí.
import { getProductById as originalGetProductById, getUserCart as originalGetUserCart, addToCart as originalAddToCart, removeFromCart as originalRemoveFromCart, clearCart as originalClearCart, checkoutCart as originalCheckoutCart, } from '../instances/odooClientInstance.js'; // USA LA MISMA RUTA QUE EN JEST.MOCK
// --- Aplicar JSDoc para ayudar al IDE/TypeScript con los tipos de los mocks ---
/** @type {jest.MockedFunction<typeof originalGetProductById>} */
const getProductById = originalGetProductById;
/** @type {jest.MockedFunction<typeof originalGetUserCart>} */
const getUserCart = originalGetUserCart;
/** @type {jest.MockedFunction<typeof originalAddToCart>} */
const addToCart = originalAddToCart;
/** @type {jest.MockedFunction<typeof originalRemoveFromCart>} */
const removeFromCart = originalRemoveFromCart;
/** @type {jest.MockedFunction<typeof originalClearCart>} */
const clearCart = originalClearCart;
/** @type {jest.MockedFunction<typeof originalCheckoutCart>} */
const checkoutCart = originalCheckoutCart;
// Nota: No necesitas castear findUserbyEmail, getProducts, etc., si no los usas directamente en este archivo de test
// pero si los usaras, seguirías el mismo patrón.
describe('Cart Resolvers with Jest', () => {
    let mockContext;
    const mockUserId = 'user-test-123';
    beforeEach(() => {
        // Ahora .mockReset() y otros métodos de mock deberían ser reconocidos
    ...
    describe('Query: getCart', () => {
        it('debería devolver el carrito del usuario con productos detallados', async () => {
            ...
            // .mockImplementation() ahora debería ser reconocido
            getProductById.mockImplementation(async (productId) => {
                if (productId === '1')
                    return [mockProductDetailsA];
                if (productId === '2')
                    return [mockProductDetailsB];
                return [];
            });
    ...
    describe('Mutation: addToCart', () => {
        ...
        it('debería llamar a odooClient.addToCart con el uid y productId correctos y devolver su resultado', async () => {
            ...
            addToCart.mockResolvedValue(mockSuccessResponse); // .mockResolvedValue también debería ser reconocido
    ...
    // ... (resto de tus pruebas, deberían funcionar de manera similar con los mocks casteados) ...
    // Asegúrate de que todas las llamadas a .mockReset(), .mockResolvedValue(), .mockImplementation(), etc.
    // usen las variables que han sido casteadas con JSDoc (getProductById, getUserCart, etc.).
    ...
    // --- Prueba de Flujo Completo del Carrito (Opcional pero Recomendado) ---
